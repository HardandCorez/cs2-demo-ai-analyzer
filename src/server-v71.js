...
const skipTrajectories = process.env.SKIP_V71_TRAJECTORIES === '1';
...
function parseTrajectorySnapshots(filePath, deaths) {
  ...
  const props = ['X','Y','Z','velocity','velocity_X','velocity_Y','last_place_name','is_alive','team_num','team_name'];
  try {
    ...
    try { return { tickRate, rows: parseTicks(filePath, ['X','Y','Z','is_alive'], ticks) }; }
    catch (fallbackError) {
...
function enrichWithTrajectories(base, filePath) {
  ...
  if (!base?.positioning || !deaths.length) {
    return { ...base, dataAvailability: { ...(base?.dataAvailability || {}), deathTrajectories: false }, advancedMetricsVersion: 'v7.1-interactive-radar' };
  }
  ...
  if (skipTrajectories) {
    return {
      ...
      positioning: {
        ...base.positioning,
        note: 'V7.1 trajectory extraction deferred to V9.2 shared replay pass. WIDE/REPEEK remain V6.1 heuristics.',
      },
      dataAvailability: { ...(base.dataAvailability || {}), deathTrajectories: false, interactiveRadar: true, trajectoryDeferred: true },
      advancedMetricsVersion: 'v7.1-interactive-radar-deferred',
    };
  }

  const { tickRate, rows } = parseTrajectorySnapshots(filePath, deaths);
  const positioning = attachDeathTrajectories({ positioning: base.positioning, deaths, snapshotRows: rows, tickRate, seconds: TRAJECTORY_SECONDS });
  const trajectoryCount = Number(positioning?.trajectory?.totalTrajectories || 0);
  return {
    ...base,
    positioning: { ...positioning, note: 'V7.1: настоящий radar + zoom/pan/click + round filters + sampled trajectory перед смертью. WIDE/REPEEK остаются эвристиками V6.1.' },
    dataAvailability: { ...(base.dataAvailability || {}), deathTrajectories: trajectoryCount > 0, interactiveRadar: true },
...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    const data = await response.json().catch(() => ({}));
    res.status(response.ok ? 200 : 502).json({ ...data, ok: response.ok && data?.ok !== false, advancedMetricsVersion: skipTrajectories ? 'v7.1-interactive-radar-deferred' : 'v7.1-interactive-radar', interactiveRadar: true, trajectoryDeferred: skipTrajectories, deathTrajectorySeconds: TRAJECTORY_SECONDS });
  } catch (error) {
...
app.post('/api/analyze', upload.single('demo'), async (req, res) => {
  ...
  try {
    ...
    res.json(enrichWithTrajectories(base, req.file.path));
  } catch (error) {
    console.error(error);
    res.status(Number(error?.status) || 422).json({ error: error?.message || 'Не удалось разобрать демку', details: error?.details, hint: error?.hint || 'Проверь консоль сервера и повтори загрузку полноценного CS2 .dem.' });
  } finally {
...
app.post('/api/ai', async (req, res) => {
  try {
    const response = await fetch(`${internalBase}/api/ai`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(req.body || {}) });
    const data = await response.json().catch(() => ({}));
...
app.listen(publicPort, '127.0.0.1', () => {
  ...
  console.log(skipTrajectories ? 'Trajectories: deferred to outer fast parser' : `Interactive radar: zoom/pan/click/round filter + ${TRAJECTORY_SECONDS}s death trajectories`);
});
